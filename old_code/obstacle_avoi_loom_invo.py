...
def process_camera_flow(camera_name, prev_image, curr_image):
    """Calculate optic flow for a single camera."""

    try:
        flow = calculate_optic_flow(prev_image, curr_image)
        if flow is not None:
...
def plot_trajectory(trajectory_fig, trajectory_ax, x_trajectory, y_trajectory, obstacle_positions, obstacle_size=0.5):
    ...
    trajectory_ax.set_xlim(0, 20)
    trajectory_ax.set_ylim(0, 20)
...
def main():
    ...
    while True:  # Limit the loop to 100 frames to avoid performance issues
        ...
        for camera_name in ["front", "left", "right", "rear"]:
            flow, loom, avg_flow = process_camera_flow(camera_name, prev_images[camera_name], curr_images[camera_name])
