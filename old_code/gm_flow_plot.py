...
alternative_rows_fb = data.iloc[::2, 10:310]  # Select columns 10:310
alternative_rows_geo = data.iloc[1::2, 10:310]  # Select columns 10:310

# Resize GMFlow data to match the same width (columns 10:310)
alternative_rows_gmflow = gmflow_data.iloc[:alternative_rows_fb.shape[0], 10:310]  # Select columns 10:310

# Check average values of the datasets
mean_fb = np.mean(alternative_rows_fb.values)
mean_geo = np.mean(alternative_rows_geo.values)
mean_gmflow = np.mean(alternative_rows_gmflow.values)

print(f"Mean Farneback Flow: {mean_fb:.6f} rad/s")
print(f"Mean Geometric Flow: {mean_geo:.6f} rad/s")
...
x_axis = np.arange(alternative_rows_fb.shape[1])  # Should be range(300) since 10:310 gives 300 values
...
ax1.set_title("FB, Geo, and GMFlow Optical Flow Comparison")
...
def init():
    ...
    frame_text.set_text('')
    return line_fb, line_geo, line_gmflow, frame_text
...
frames = min(len(alternative_rows_fb), len(alternative_rows_geo), len(alternative_rows_gmflow))  # Match frame count
