...
# obstacles position
obstacle_positions = [(-3.77, 2.85), (-5.97, 8.55), (-1.77, 7.05), (-1.37, 13.65), (-0.17, 21.35), (6.83, 23.75),
                      (4.03, 16.55), (2.43, 10.05), (1.63, 2.85), (4.03, -1.95), (8.53, -0.15), (5.73, 4.05),
                      (6.33, 8.55), (9.03, 11.45), (9.03, 16.95), (11.43, 23.75), (14.43, 16.95), (11.73, 9.25),
                      (9.03, 4.05), (12.33, 1.65)]
...
def calculate_velocity_commands(V_tgt_x, V_tgt_y, L_A, L_F, L_L, L_R, Q_A, Q_F, Q_L, Q_R, V_x, V_y, k0=4, k1=5,
                                MAX_SPEED=1):
...
def main():
    ...
    #client.moveToZAsync(-4, 1.5).join()
    print('TAKEN OFF')
    #client.moveByVelocityAsync(0,0,-1,1).join()# z is down
    #client.moveByVelocityAsync(1, 0, 0, 1).join() # x is forward
    #client.moveByVelocityAsync(0, 1, 0, 1).join() # y is right
    print("HOME")
    client.moveToPositionAsync(0,0,-1,1.5).join()
    print("STARTING ROLLING ")
    client.moveByRollPitchYawrateThrottleAsync(1/180,0,0,.75,1.) # rolls along +x axis, 1 is ccw
    quit()
    ...
    while True:  # Limit the loop to 100 frames to avoid performance issues
        ...
        for camera_name in ["front", "left", "right", "rear"]:
            flow, loom, avg_flow = process_camera_flow(client, camera_name, prev_images[camera_name],
                                                       curr_images[camera_name])
